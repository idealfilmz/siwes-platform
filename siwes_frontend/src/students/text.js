...
function TextPage() {
  ...
  const Getdetails = async () => {
    try {
      const response = await fetch(
        `http://127.0.0.1:3000/fetch-all-anounve?student_id=${std_id}`,
        {
          method: "GET",
          headers: {
  ...
  return (
    <div>
      <h1 className="text-gray-600 text-center p-1 m-1 text-2xl">
      ...
      {data.length === 0 ? (
        ...
        <div>
          ...
          <button
            onClick={checkAnswer}
            className="text-xl bg-blue-950 text-white rounded-sm p-3 m-2 text-center"
            type="submit"
          >
            Submit Answers
          </button>
